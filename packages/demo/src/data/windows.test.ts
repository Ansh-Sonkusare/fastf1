...
describe("positionAt", () => {
  const tracks = indexLocations([loc(1, 0.25, 100, 0), loc(4, 0, 0, 0), loc(1, 0, 0, 0), loc(1, 0.5, 100, 100), loc(1, 5, 0, 0)]);

  it("interpolates between the samples around the instant, out of order input included", () => {
    expect(positionAt(tracks.get(1)!, T + 125, T + 1_000)).toEqual([50, 0]);
    expect(positionAt(tracks.get(1)!, T + 375, T + 1_000)).toEqual([100, 50]);
  });
  ...
  it("has no position across a gap or outside the samples", () => {
    ...
    expect(positionAt(tracks.get(4)!, T, T)).toEqual([0, 0]);
  });
